...
public class CreateCompletableFutureMain {
	...
	public static void supplyAsyncWithExecutor() {
		...
		try {
			Supplier<String> supplier = () -> "test";
			CompletableFuture.supplyAsync(supplier, executor).join();
		} catch (Exception e) {
	...
	public static void thenApplyAsyncWithExecutor() {
		...
		try {
			...
			future.thenApplyAsync(function, executor).join();
		} catch (Exception e) {
			throw new RuntimeException(e);
		} finally {
			executor.shutdown();
	...
	/**
	 * Tests CompletableFuture.thenCombineAsync(CompletionStage, BiFunction,
	 * Executor) method
	 */
