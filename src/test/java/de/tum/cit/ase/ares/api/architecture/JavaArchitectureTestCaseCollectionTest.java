...
class JavaArchitectureTestCaseCollectionTest {
    ...
    void testNoClassShouldAccessFileSystem() {
        ...
        assertDoesNotThrow(() -> JavaArchUnitTestCaseCollection.NO_CLASS_SHOULD_ACCESS_FILE_SYSTEM.check(classes));
    }

    @Test
    void testNoClassesShouldAccessNetwork() {
        JavaClasses classes = new ClassFileImporter().importPackages("com.example");
        assertDoesNotThrow(() -> JavaArchUnitTestCaseCollection.NO_CLASSES_SHOULD_ACCESS_NETWORK.check(classes));
    }

    @Test
    void testNoClassesShouldTerminateJvm() {
        JavaClasses classes = new ClassFileImporter().importPackages("com.example");
        assertDoesNotThrow(() -> JavaArchUnitTestCaseCollection.NO_CLASSES_SHOULD_TERMINATE_JVM.check(classes));
    }
}
