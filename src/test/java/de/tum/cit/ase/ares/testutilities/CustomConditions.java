...
public final class CustomConditions {
	...
	public enum Option {
		MESSAGE_CONTAINS, MESSAGE_REGEX, MESSAGE_NORMALIZE_NEWLINE
	}
	...
	public static Condition<? super Event> testFailedWith(String testName, Class<? extends Throwable> errorType, Option... options) {
		return testFailedWith(testName, errorType, NO_MSG, null, options);
	}

	public static Condition<? super Event> testFailedWith(String testName, Class<? extends Throwable> errorType, String message, Option... options) {
		return testFailedWith(testName, errorType, message, null, options);
	}

	public static Condition<? super Event> testFailedWith(String testName, Class<? extends Throwable> errorType, String message, Condition<Throwable> customCondition, Option... options) {
		...
		return event(testWithSegments(testName), EventConditions.finished(new Condition<>(testFailRespectingDirectFailure(testName, conditions), "status is not SUCCESSFUL"))); //$NON-NLS-1$
	}
	...
	private static Condition<Throwable> createMessageCondition(String message, EnumSet<Option> optionSet) {
		...
		if (messageContains)
			return createMessageCondition(message, optionSet, messageForComparison -> assertThat(messageForComparison).contains(message),
					messageForComparison -> messageForComparison != null && messageForComparison.contains(message), "message contains '%s'"); //$NON-NLS-1$
		if (messageRegex)
			return createMessageCondition(message, optionSet, messageForComparison -> assertThat(messageForComparison).matches(message),
					messageForComparison -> messageForComparison != null && messageForComparison.matches(message), "message matches the regex '%s'"); //$NON-NLS-1$
		return createMessageCondition(message, optionSet, messageForComparison -> assertThat(messageForComparison).isEqualTo(message),
				messageForComparison -> Objects.equals(messageForComparison, message), "message is '%s'"); //$NON-NLS-1$
	}

	private static Condition<Throwable> createMessageCondition(String message, EnumSet<Option> optionSet, Consumer<String> directlyFailingAssertion, Predicate<String> assertionAsBoolean,
	...
	private static String normalizeMessageNewLines(EnumSet<Option> optionSet, String actualMessage) {
		if (actualMessage == null)
			return null;
		return optionSet.contains(Option.MESSAGE_NORMALIZE_NEWLINE)
				? actualMessage.replaceAll("\\R", "\n") //$NON-NLS-1$ //$NON-NLS-2$
				: actualMessage;
	}

	private static Predicate<TestExecutionResult> testFailRespectingDirectFailure(String testName, ArrayList<Condition<Throwable>> conditions) {
	...
	public static Condition<Event> finishedSuccessfully(String testName) {
		return event(testWithSegments(testName), EventConditions.finished(new Condition<>(testSuccessRespectingDirectFailure(testName), "status is SUCCESSFUL"))); //$NON-NLS-1$
	}
	...
	private static AssertionFailedError generateFailure(String testName, TestExecutionResult testExecutionResult, Condition<Throwable> requirements, Throwable t) {
	...
	private static void truncateStackTrace(AssertionError e) {
		e.setStackTrace(Stream.of(e.getStackTrace()).limit(CONDITION_FAIL_STACK_LIMIT).toArray(StackTraceElement[]::new));
	}
