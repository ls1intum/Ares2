...
public class JavaResourceLimitsExtractor implements ResourceLimitsExtractor {
	...
	/**
	 * Constructs a new JavaResourceLimitsExtractor with the specified resource
	 * access supplier.
	 *
	...
	/**
	 * Retrieves the tightest timeout limit given by the Java security policy (in
	 * case multiple timeouts are configured).
	 *
	 * @return the minimum timeout value in milliseconds, or a restrictive default
	 ...
	 *
	 * @return a map of resource limits where the key is the resource type and the
	 *         value is the limit.
	 */
	@Nonnull
	...
	private static List<ResourceLimitsPermission> validatePermissions(List<?> rawList) {
		if (rawList == null) {
			throw new SecurityException(Messages.localized("security.resource.limits.permission.null.list"));
		}
		...
		for (Object item : rawList) {
			if (item == null) {
				throw new SecurityException(Messages.localized("security.resource.limits.permission.null.entry"));
			}
