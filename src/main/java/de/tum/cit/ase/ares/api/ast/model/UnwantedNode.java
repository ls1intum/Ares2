...
public class UnwantedNode {
	...
	public static List<UnwantedNode> getUnwantedNodesInJavaFile(JavaFile javaFile, Map<String, Class<? extends Node>> nodesDefinedAsUnwanted) {
		return nodesDefinedAsUnwanted.keySet().stream().map(unwantedNodeName -> new UnwantedNode(javaFile, unwantedNodeName, nodesDefinedAsUnwanted.get(unwantedNodeName)))
				.filter(unwantedNode -> !unwantedNode.getUnwantedNodePositions().isEmpty()).sorted(Comparator.comparing(uwn -> uwn.getUnwantedNodePositions().get(0))).collect(Collectors.toList());
	}

	/**
	 * Detects a provided list of unwanted nodes in a Java-File at a given path
	 *
	 * @param pathOfJavaFile Path to the Java-File, where unwanted nodes shall be
	 *            detected
	...
	public static String getFormattedUnwantedNodeString(UnwantedNode unwantedNode) {
		return unwantedNode.getUnwantedNodePositions().stream().map(String::valueOf).map(UnwantedNode::getFormattedPositionString).collect(
				Collectors.joining(System.lineSeparator(), localized("ast.method.get_formatted_unwanted_node_string_prefix", unwantedNode.getUnwantedNodeName()) + System.lineSeparator(), ""));
	}
	...
	public static Optional<String> getMessageForUnwantedNodesForFileAt(Path pathOfJavaFile, Map<String, Class<? extends Node>> nodeNameUnwantedNodeMap, boolean excludeMainMethod) {
		Map<Path, List<UnwantedNode>> unwantedNodes = getUnwantedNodesForFileAt(pathOfJavaFile, nodeNameUnwantedNodeMap, excludeMainMethod);
		if (unwantedNodes.isEmpty()) {
	 ...
	 * Creates an error message in case unwanted files are detected
	 *
	 * @param pathOfDirectory Path to the Directory, at and below where unwanted
	 *            nodes shall be detected
	 * @param nodeNameUnwantedNodeMap List of unwanted nodes
	 * @return Error message
	 */
	public static Optional<String> getMessageForUnwantedNodesForAllFilesBelow(Path pathOfDirectory, Map<String, Class<? extends Node>> nodeNameUnwantedNodeMap, boolean excludeMainMethod) {
		return JavaFile.readFromDirectory(pathOfDirectory, excludeMainMethod).stream().sorted(Comparator.comparing(JavaFile::getJavaFilePath))
				.map(javaFile -> getMessageForUnwantedNodesForFileAt(javaFile.getJavaFilePath(), nodeNameUnwantedNodeMap, excludeMainMethod)).filter(Optional::isPresent).map(Optional::get)
