...
public class TransitivelyAccessesMethodsCondition extends ArchCondition<JavaClass> {
	...
	private static ConditionEvent newTransitiveAccessPathFoundEvent(JavaAccess<?> access, List<JavaAccess<?>> transitiveAccessDependencyPath) {
		...
		if (hasTransitiveDependencyPath) {
			...
			String joinedChain = transitiveAccessDependencyPath.stream().map(transitiveAccess -> transitiveAccess.getOrigin().getFullName()).collect(Collectors.joining("->"));
	...
	private class TransitiveAccessPath {
		 ...
		 */
		private boolean recursivelyFindPathFromViolatingMethodTo(JavaAccess<?> access, ImmutableList.Builder<JavaAccess<?>> transitiveAccessPathBuilder, Set<String> namesOfAnalyzedMethods) {
