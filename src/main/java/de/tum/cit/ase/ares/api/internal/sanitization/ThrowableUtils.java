...
final class ThrowableUtils {
	...
	static final Set<String> THROWABLE_PROPERTIES = Stream.concat(IGNORE_PROPERTIES.stream(), Stream.of(MESSAGE, STACK_TRACE, CAUSE, SUPPRESSED)).collect(Collectors.toUnmodifiableSet());
	...
	static Comparator<Constructor<?>> getConstructorPreferenceOrder() { // NOSONAR
		return Comparator.<Constructor<?>>comparingInt(Constructor::getParameterCount).thenComparingLong(x -> Stream.of(x.getParameterTypes()).filter(String.class::equals).count()).reversed();
	}
	...
	static Map<Class<?>, List<Object>> getValuesByPropertyType(Stream<Entry<String, Method>> propertiesWithMethods, Map<String, Object> concretePropertyValues) {
		return propertiesWithMethods
				.collect(Collectors.groupingBy(property -> property.getValue().getReturnType(), Collectors.mapping(property -> concretePropertyValues.get(property.getKey()), Collectors.toList())));
	}
