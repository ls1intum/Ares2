...
public class JavaCommandSystemExtractor implements CommandSystemExtractor {
	...
	/**
	 * Constructs a new JavaCommandExecutionExtractor with the specified resource
	 * access supplier.
	 *
	...
	public static List<String> extractCommands(@Nonnull List<CommandPermission> configs) {
		return configs.stream().map(CommandPermission::executeTheCommand).toList();
	}

	/**
	 * Extracts the permitted arguments for command execution from the provided
	 * configurations.
	 *
	 * @param configs the list of JavaTestCase configurations, must not be null.
