...
public class JavaThreadSystemExtractor implements ThreadSystemExtractor {
	...
	/**
	 * Constructs a new JavaThreadCreationExtractor with the specified resource
	 * access supplier.
	 *
	...
	public static List<String> extractThreadNumbers(@Nonnull List<ThreadPermission> configs) {
		return configs.stream().map(ThreadPermission::createTheFollowingNumberOfThreads).map(String::valueOf).toList();
	}
	...
	public static List<String> extractThreadClasses(@Nonnull List<ThreadPermission> configs) {
		return configs.stream().map(ThreadPermission::ofThisClass).toList();
	}
