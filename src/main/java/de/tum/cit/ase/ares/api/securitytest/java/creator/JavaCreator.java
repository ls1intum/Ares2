...
public class JavaCreator implements Creator {
	 ...
	 */
	@Nonnull
	private Set<PackagePermission> prepareAllowedPackages(@Nonnull List<String> essentialPackages, @Nonnull ResourceAccesses resourceAccesses, @Nonnull String packageName) {
		return Stream.of(
	 ...
	 */
	@Nonnull
	private Set<ClassPermission> prepareAllowedClasses(@Nonnull List<String> essentialClasses, @Nonnull List<String> testClasses) {
		return Stream.of(
				...
				testClasses.stream().map(ClassPermission::new)).flatMap(Function.identity()).collect(Collectors.toSet());
	}
	 ...
	 */
	@Nonnull
	private JavaArchitectureTestCase createArchitectureTestCase(@Nonnull JavaArchitectureTestCaseSupported supported, @Nonnull JavaClasses classes, @Nonnull CallGraph callGraph,
	 ...
	 */
	@Nonnull
	private JavaAOPTestCase createAOPTestCase(@Nonnull ResourceAccesses resourceAccesses, @Nonnull List<ArchitectureTestCase> javaArchitectureTestCases, @Nonnull JavaAOPTestCaseSupported supported,
			@Nonnull JavaClasses classes, @Nonnull CallGraph callGraph, @Nonnull Set<PackagePermission> allowedPackages, @Nonnull Set<ClassPermission> allowedClasses) {
		@Nonnull
		Supplier<List<?>> resourceAccessSupplier = List.of((Supplier<List<?>>) resourceAccesses::regardingFileSystemInteractions, resourceAccesses::regardingNetworkConnections,
				resourceAccesses::regardingCommandExecutions, resourceAccesses::regardingThreadCreations).get(supported.ordinal());
	...
	private PhobosTestCase createPhobosTestCase(ResourceAccesses resourceAccesses, JavaPhobosTestCaseSupported supported) {
		...
		Supplier<List<?>> resourceAccessSupplier = List
				.of((Supplier<List<?>>) resourceAccesses::regardingFileSystemInteractions, resourceAccesses::regardingNetworkConnections, resourceAccesses::regardingTimeouts).get(supported.ordinal());

		return JavaPhobosTestCase.builder().javaPhobosTestCaseSupported(supported).resourceAccessSupplier(resourceAccessSupplier).build();
	 ...
	 * @param allowedClasses the set of allowed class names; must not be null
	 */
	private void addFixedTestCases(@Nonnull List<ArchitectureTestCase> javaArchitectureTestCases, @Nonnull JavaClasses classes, @Nonnull CallGraph callGraph,
			@Nonnull Set<PackagePermission> allowedPackages, @Nonnull Set<ClassPermission> allowedClasses) {
		javaArchitectureTestCases.addAll(JavaArchitectureTestCaseSupported
				...
				// interface)
				.TERMINATE_JVM.getStatic().stream().map(fixedCase -> createArchitectureTestCase((JavaArchitectureTestCaseSupported) fixedCase, classes, callGraph, allowedPackages, allowedClasses))
						.toList());
	}
	 ...
	 * @param javaArchitectureTestCases the list to populate with architecture test
	 *            cases; must not be null
	 * @param javaAOPTestCases the list to populate with AOP test cases; must not be
	 *            null
	 * @param classes the Java classes to analyze; must not be null
	 * @param callGraph the call graph to analyze; must not be null
	 * @param allowedPackages the set of allowed package permissions; must not be
	 ...
	 * @param allowedClasses the set of allowed class names; must not be null
	 */
	private void addVariableTestCases(@Nonnull List<ArchitectureTestCase> javaArchitectureTestCases, @Nonnull List<AOPTestCase> javaAOPTestCases, @Nonnull List<PhobosTestCase> javaPhobosTestCases,
			@Nonnull JavaClasses classes, @Nonnull CallGraph callGraph, @Nonnull Set<PackagePermission> allowedPackages, @Nonnull Set<ClassPermission> allowedClasses,
		...
		javaAOPTestCases.addAll(JavaAOPTestCaseSupported
				...
				// interface)
				.FILESYSTEM_INTERACTION.getDynamic().stream().map(
						variableCase -> createAOPTestCase(resourceAccesses, javaArchitectureTestCases, (JavaAOPTestCaseSupported) variableCase, classes, callGraph, allowedPackages, allowedClasses))
						.toList());
	 ...
	 */
	public void createTestCases(@Nonnull BuildMode buildMode, @Nonnull ArchitectureMode architectureMode, @Nonnull AOPMode aopMode, @Nonnull List<String> essentialPackages,
			...
			@Nonnull List<ArchitectureTestCase> architectureTestCases, @Nonnull List<AOPTestCase> aopTestCases, @Nonnull List<PhobosTestCase> phobosTestCases,
		...
		@Nonnull
		JavaClasses javaClasses = cacheResult(projectPath + "_javaClasses", () -> architectureMode.getJavaClasses(classPath)).get();
		@Nonnull
		CallGraph callGraph = cacheResult(projectPath + "_callGraph", () -> architectureMode.getCallGraph(classPath)).get();
		// </editor-fold>

		// <editor-fold desc="Preparation">
		@Nonnull
		Set<PackagePermission> allowedPackages = prepareAllowedPackages(essentialPackages, resourceAccesses, packageName);
		@Nonnull
