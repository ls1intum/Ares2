...
/**
 * Block JNDI lookups from student code in BLOCKED_ALL mode.
 *
 * <p>Description: ArchUnit catches JNDI references statically; WALA does not. Under
 * {@code WALA_AND_ASPECTJ}, JNDI operations from the restricted student package fall
 * through to here and are rejected with {@link SecurityException}.
 */
@SuppressWarnings("AopLanguageInspection")
public aspect JavaAspectJJndiAdviceDefinitions extends JavaAspectJAbstractAdviceDefinitions {

    before(): JavaAspectJJndiPointcutDefinitions.jndiMethods() {
        ...
        throw new SecurityException(
                "Ares Security Error (Reason: Student-Code; Stage: Execution): "
                        + systemMethodToCheck + " tried to illegally perform a JNDI lookup via "
                        + fullMethodSignature
