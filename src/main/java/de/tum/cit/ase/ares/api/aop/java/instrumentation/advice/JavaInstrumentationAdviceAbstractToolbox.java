...
public abstract class JavaInstrumentationAdviceAbstractToolbox {
	...
	JavaInstrumentationAdviceAbstractToolbox() {
		throw new SecurityException(JavaInstrumentationAdviceAbstractToolbox.localize("security.instrumentation.utility.initialization", "JavaInstrumentationAdviceAbstractToolbox"));
	}

	// <editor-fold desc="Constants">
	@Nonnull
	public static final List<String> IGNORE_CALLSTACK = List.of("java.lang.ClassLoader", "de.tum.cit.ase.ares.api.", "com.intellij.rt.debugger.");
	// </editor-fold>
	...
	public static <T> T getValueFromSettings(@Nonnull String fieldName) {
		try {
			...
			// operations
			@Nonnull
			Class<?> adviceSettingsClass = Objects.requireNonNull(Class.forName("de.tum.cit.ase.ares.api.aop.java.JavaAOPTestCaseSettings", false, null), "adviceSettingsClass must not be null");
			@Nonnull
			Field field = Objects.requireNonNull(adviceSettingsClass.getDeclaredField(Objects.requireNonNull(fieldName, "fieldName must not be null")), "field must not be null");
	...
	public static Object getSettingsLock() {
		try {
			...
			try {
				...
				throw new SecurityException(JavaInstrumentationAdviceAbstractToolbox.localize("security.advice.class.not.found.exception", "JavaAOPTestCaseSettings"), ex);
			}
		}
	}
	 ...
	 *
	 * <p>
	 * Description: Examines the stack trace to find the first element whose class
			...
			case "ALL_EXCEPT" :
				if (ignoreVariables.getIndex() < 0 || ignoreVariables.getIndex() >= newVariables.size()) {
					throw new SecurityException(
							JavaInstrumentationAdviceAbstractToolbox.localize("security.instrumentation.ignore.values.index.invalid", ignoreVariables.getIndex(), newVariables.size()));
				}
				@Nonnull
