...
public abstract class SecurityPolicyDirector {
	 ...
	 *            not be null.
	 */
	public SecurityPolicyDirector(@Nonnull Creator creator, @Nonnull Writer writer, @Nonnull Executer executer, @Nonnull EssentialDataYAMLReader essentialDataReader,
			@Nonnull ProjectScanner projectScanner, @Nonnull Path essentialPackagesPath, @Nonnull Path essentialClassesPath) {
	...
	public static SecurityPolicyDirector selectSecurityPolicyDirector(@Nullable SecurityPolicy securityPolicy) {
		if (securityPolicy == null) {
			return SecurityPolicyJavaDirector.javaBuilder().creator(new JavaCreator()).writer(new JavaWriter()).executer(new JavaExecuter()).essentialDataReader(new EssentialDataYAMLReader())
					.javaScanner(new JavaProjectScanner()).essentialPackagesPath(SecurityPolicyJavaDirector.DEFAULT_ESSENTIAL_PACKAGES_PATH)
			...
			return switch (securityPolicy.regardingTheSupervisedCode().theFollowingProgrammingLanguageConfigurationIsUsed()) {
				case JAVA_USING_GRADLE_ARCHUNIT_AND_ASPECTJ, JAVA_USING_GRADLE_ARCHUNIT_AND_INSTRUMENTATION, JAVA_USING_GRADLE_WALA_AND_ASPECTJ, JAVA_USING_GRADLE_WALA_AND_INSTRUMENTATION, JAVA_USING_MAVEN_ARCHUNIT_AND_ASPECTJ, JAVA_USING_MAVEN_ARCHUNIT_AND_INSTRUMENTATION, JAVA_USING_MAVEN_WALA_AND_ASPECTJ, JAVA_USING_MAVEN_WALA_AND_INSTRUMENTATION -> SecurityPolicyJavaDirector
						.javaBuilder().creator(new JavaCreator()).writer(new JavaWriter()).executer(new JavaExecuter()).essentialDataReader(new EssentialDataYAMLReader())
						.javaScanner(new JavaProjectScanner()).essentialPackagesPath(SecurityPolicyJavaDirector.DEFAULT_ESSENTIAL_PACKAGES_PATH)
						.essentialClassesPath(SecurityPolicyJavaDirector.DEFAULT_ESSENTIAL_CLASSES_PATH).build();
			};
		}
