...
public class JavaPhobosTestCase extends PhobosTestCase {
	 ...
	 * later code that relies on declaration order (e.g. an <code>LD_PRELOAD</code>
	 * helper) still works.
	 * </p>
	 *
	...
	/**
	 * Appends a formatted “[network…]” policy section to {@code out}. The
	 * {@code hostsAndPorts} set must already contain fully formatted
	 * {@code host[:port]} strings (exactly what {@link #collectAllowHostsAndPorts}
	...
	private static void appendNetworkSection(StringBuilder out, String header, Set<String> hostsAndPorts) {
		...
		hostsAndPorts.stream().sorted().forEach(hp -> out.append("allow ").append(hp).append('\n'));
