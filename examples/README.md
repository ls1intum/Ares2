...
Two complete, minimal exercises that build and run as they stand. They exist so that setting up Ares does not start from a blank file, and so that the snippets in [the setup manual](../docs/HowToMakeAProjectAnAresProject.md) are demonstrably correct rather than merely plausible.
