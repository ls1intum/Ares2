...
- `src/test/java/de/tum/cit/ase/ares/integration/aop/` holds the largest set by far. Under `forbidden/subject/`, classes attempt file reads, writes, creations, deletions and executions, socket and HTTP connections, command execution through `Runtime` and `ProcessBuilder`, and thread creation through many different APIs, each so that a policy can block it. Under `allowed/subject/`, classes attempt the same operations under a policy that permits them, so that an operation wrongly blocked is caught as well as one wrongly permitted. The tests that drive them live in `integration/aop/` and in `integration/architecture/`, which holds tests only.
...
Some of this code trips automated scanners. A scanner reads a fixture without knowing that it is one, so a finding in these trees is a question about the fixture rather than a report about Ares.
