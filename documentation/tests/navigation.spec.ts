...
test.describe('the two guides', () => {
    ...
    test('each guide renders every one of its top-level sections', async ({ page }) => {
        ...
        for (const section of [
            'Ares 2',
            'How to protect a Java project with Ares 2',
            'Policy Cookbook',
            'Policy Reference',
            'Troubleshooting',
        ]) {
            await expect(sidebar.getByText(section, { exact: true }), `instructor sidebar shows ${section}`).toBeVisible();
        }
        ...
        for (const section of [
            ...
            'Technologies on which Ares 2 is built',
            'Policy',
            'Subsystems',
            'Extending Ares 2',
        ]) {
    ...
    test('a page that is only a scaffold says so', async ({ page }) => {
        // Stubs are deliberately listed rather than unlisted, so that the information
        // architecture can be reviewed. That is only defensible while a reader can tell a stub
        // from a finished page at a glance, which is what this asserts.
        await page.goto('./contributor/extending-ares-2/new-build-tools');
        const stubNotice = page.locator('.theme-admonition').filter({ hasText: 'This page is a stub' });
        await expect(stubNotice).toHaveCount(1);
        await expect(stubNotice).toBeVisible();
    });
