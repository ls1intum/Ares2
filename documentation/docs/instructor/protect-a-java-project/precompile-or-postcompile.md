...
1. **Create a security policy and annotate tests:** follow the [Security Policy Manual](/contributor/subsystems/policy/security-policy-manual), which explains how to write `SecurityPolicy.yaml` files and apply `@Policy` to your tests. If your exercise needs no resource access at all, [Further Options](./further-options.md) describes the alternative.
