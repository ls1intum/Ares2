...
| Policy seems to have no effect | Wrong `withinPath` | Gradle: `classes/java/main/<package/path>`; Maven: `classes/<package/path>` |
| A `@StrictTimeout` was replaced by `regardingTimeouts` and no longer bounds anything | Timeouts are Phobos cases; Ares 2.1.5 generates the Phobos stage but does not yet dispatch it in-process | Restore `@StrictTimeout` (the step named in section 6.2 of this guide) |
