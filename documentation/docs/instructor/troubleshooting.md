...
| `[Xlint:adviceDidNotMatch]` warnings during compilation | An `_ASPECTJ` build reports each Ares advice that found no matching join point in the woven code | Expected and non-fatal when the exercise performs none of those operations. Their presence shows the weaver processed the Ares aspects, so do not suppress the category |
...
| A Gradle file-system watcher (FSEvents) warning at build start | Gradle's native file watching could not start in this environment | A warning only; it does not affect compilation or tests. Pass `--no-watch-fs` to silence it |
...
| `package de.tum.in.test.api does not exist` | An Ares 1 import survived the rewrite | Search for `de.tum.in.test` across the test sources. Any remaining hit is either a rename (section 5 of this guide) or a security annotation to translate (section 6 of this guide) |
...
| Policy seems to have no effect | Wrong `withinPath` | Gradle: `classes/java/main/<package/path>`; Maven: `classes/<package/path>` |
| A `@StrictTimeout` was replaced by `regardingTimeouts` and no longer bounds anything | Timeouts are Phobos cases; Ares 2.1.4 generates the Phobos stage but does not yet dispatch it in-process | Restore `@StrictTimeout` (the step named in section 6.2 of this guide) |
