...
The eight accepted values are the full cross product of the three dimensions: `MAVEN`/`GRADLE` × `ARCHUNIT`/`WALA` (the T. J. Watson Libraries for Analysis) × `ASPECTJ`/`INSTRUMENTATION`. Continuous integration (CI) exercises all four analysis and weaving combinations on every change.
