...
:::tip[Simple Story]
When a tool reads a pupil's source, it does not keep it as text. It turns it into a tree.

The whole file is the trunk, each class is a branch, each method a smaller branch, each
statement a twig. A node is any one of those. Working on a tree is far easier than working on
text, because the structure is already worked out.
:::
