...
| Timeouts | Declared maximum execution time. **Not enforced by the JUnit extension path in Ares 2.1.5**; use `@StrictTimeout` for an actual deadline | Record an intended limit of 10 seconds |
...
Declares an intended maximum execution time in milliseconds. **In Ares 2.1.5 this declaration does not yet take effect**; see the note below.
...
| `timeout` | Must be **> 0** (the restrictive default is 3000 ms). Validated on load; does not yet take effect at run time, see [Section 8.6](#86-timeout-permissions) | `IllegalArgumentException` with a localised message |
