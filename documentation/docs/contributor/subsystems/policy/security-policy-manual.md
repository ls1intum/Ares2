...
| Timeouts | Declared maximum execution time. **Not enforced by the JUnit extension path in Ares 2.1.3**; use `@StrictTimeout` for an actual deadline | Record an intended limit of 10 seconds |
...
Declares an intended maximum execution time in milliseconds. **In Ares 2.1.3 this declaration does not yet take effect**; see the note below.
