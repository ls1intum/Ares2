...
def strip_generics(s):
    # remove simple generics to make parsing supertypes easier
    while True:
        new = re.sub(r"<[^<>]*>", "", s)
        if new == s:
            return new
...
def analyze():
    ...
    return entries, duplicates, coverage, cache


def main():
    entries, duplicates, coverage, cache = analyze()

    print("=== READ OPERATIONS (2.2) ANALYSIS ===")
    ...
    # Optional: report methods not found in javap output
