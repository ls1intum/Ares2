...
The handled first characters are exactly `[`, space, tab, carriage return, vertical tab and form feed, in each of the `readonly`, `read`, `write`, `hide` and `tmpfs` sections. Absolute paths, and entries whose brackets or blanks appear later such as `relative/[draft` or `/tmp/a[b]c`, need no change. Phobos policies generated by Ares apply this serialisation automatically, so only hand-written configurations are concerned.
