...
### Native configuration: bracket-leading and whitespace-leading filesystem entries

The bundled Phobos parser reserves any line whose first meaningful character is `[` for a section header, and it trims leading blank characters off every line before reading it. A relative filesystem entry beginning with `[` or with a blank character therefore needs an explicit `./` prefix when it is written by hand in a `Base*.cfg` file or in a file passed through `--config`: write `./[draft` rather than `[draft`, and `./ [draft` rather than ` [draft`. Both spellings name the same relative location once the wrapper canonicalises the path, so the prefix records the intended meaning rather than changing it.

The handled first characters are exactly `[`, space, tab, carriage return, vertical tab and form feed, in each of the `readonly`, `read`, `write`, `hide` and `tmpfs` sections. Absolute paths, and entries whose brackets or blanks appear later such as `relative/[draft` or `/tmp/a[b]c`, need no change. Phobos policies generated by Ares apply this serialisation automatically, so only handwritten configurations are concerned.
